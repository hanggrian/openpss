...
class AboutDialog(private val component: Component) : Dialog<Unit>(), Component by component {
    ...
    init {
        ...
        content = hbox {
            ...
            vbox {
                ...
                textFlow {
                    text("Core  ") { font = loadFont(latoBold, 12.0) }
                    text("${getString(R.string.version)} ${com.wijayaprinting.BuildConfig.VERSION}") { font = loadFont(latoRegular, 12.0) }
                } marginTop 4
                textFlow {
                ...
                hbox {
                    button("GitHub") { setOnAction { browse(WP.getGitHub()) } }
                    button(getString(R.string.check_for_updates)) { setOnAction { browse(WP.getGitHub("wp-manager/releases")) } } marginLeft 8
                } marginTop 20
            } marginLeft 48
        }
    ...
    private val licenses
        get(): ObservableList<License> = License.listAll().addAllSorted(License.COMPARATOR,
