        ...
        withContext(Dispatchers.Default) {
            ...
            file.inputStream().use { stream ->
                XSSFWorkbook(stream).use { workbook ->
                    workbook
                        .getSheetAt(SHEET_RAW_ATTENDANCE_LOGS)
                        .iterator()
                        .asSequence()
                        .drop(5)
                        .forEach { row ->
                            ...
                            val date =
                                LocalDate.fromDateFields(row.getCell(CELL_DATE).dateCellValue)
                            multimap.putAll(
                                ...
                                (CELL_RECORD_START until CELL_RECORD_END)
                                    ...
                                    .map {
                                        date.toDateTime(LocalTime.fromDateFields(it.dateCellValue))
                                    },
                            )
                        }
                }
